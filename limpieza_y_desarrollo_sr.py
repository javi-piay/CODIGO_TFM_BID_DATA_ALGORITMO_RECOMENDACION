# -*- coding: utf-8 -*-
"""Limpieza y Desarrollo_SR.ipynb
...
"""# 📊 Capítulo 6: Modelo de Recomendación Híbrido

## ¿En qué consiste este modelo?

En este capítulo desarrollamos un **sistema híbrido de recomendación** basado en el contenido de los restaurantes.  
Dado que no disponemos de datos de usuarios individuales (necesarios para un filtrado colaborativo clásico), nos centramos en un híbrido **dentro del enfoque content-based**, que combina varios mecanismos:

1. **🔎 Búsqueda por texto libre**  
   El usuario puede introducir consultas (*ej: “pulpo gallego barato”*) y encontrar restaurantes relevantes a partir de descripciones, reseñas y categorías.

2. **🍔 Filtros por categorías, flags y subcategorías**  
   El sistema permite activar filtros como *vegano, hamburguesas, italiana, gourmet*, etc. (`flag_*`), así como subcategorías más específicas (*ej: `subcat_española_pulpo`, `subcat_veggie_sin_gluten`*).

3. **📊 Ordenación personalizada**  
   Los resultados pueden ordenarse por criterios como `rating_final`, `reviews_final`, `price_num` o la métrica interna `score`.

4. **📍 Proximidad geográfica y horarios**  
   Además, se incorporan filtros por distancia (Haversine o rutas reales con ORS) y disponibilidad horaria (`horarios_norm`), para que el usuario vea solo restaurantes cercanos y abiertos.

Este **enfoque mixto** combina búsqueda semántica, filtrado, ordenación y proximidad, ofreciendo una experiencia flexible y realista de exploración gastronómica.


"""

# --- Paso 1: Cargar dataset y librerías ---
...
"""## Preprocesamiento para búsqueda por texto libre (TF-IDF)

El objetivo es permitir al usuario buscar restaurantes introduciendo un texto libre (*ej: “pulpo gallego barato”*).  
...
2. Aplicamos **TF-IDF (Term Frequency - Inverse Document Frequency)** para vectorizar ese texto.  
   - Cada restaurante se convierte en un vector numérico que representa sus palabras clave.  
3. Calculamos similitudes de coseno entre la consulta del usuario y los restaurantes.  
   - Cuanto más alto el valor, más relevante es el restaurante respecto a la búsqueda.  

"""

# 1. Crear un campo combinado de texto más completo
...
# Crear el vectorizador con stopwords en español
from sklearn.feature_extraction.text import TfidfVectorizer
...
print("✅ Matriz TF-IDF creada:", tfidf_matrix.shape)
...
def explicar_resultados_tfidf(query, top_n=5):
    """
    Explica por qué aparecen los restaurantes en los resultados TF-IDF:
    - Muestra coincidencias literales con la query
    - Indica los scores de similitud
    - Visualiza las palabras más relevantes del top-1
    """
    ...
    print(f"\n🔎 **Consulta:** {query}")
    print("📊 Resultados más similares por TF-IDF:\n")
    ...
    for i, idx in enumerate(indices):
        ...
        print(f"{i+1}. {nombre} ({ciudad}) → similitud: {score:.4f}")
        if coincidencias:
            print(f"   💬 Coincidencias literales: {', '.join(coincidencias)}")
        else:
            print("   ⚙️ Coincidencia contextual (no literal)")
        print()

    # --- Visualización de palabras más relevantes del top-1 ---
    idx_top = indices[0]
    ...
    plt.title(f"🔠 Palabras con mayor peso TF-IDF en '{df.iloc[idx_top]['nombre_final']}'")
...
def buscar_hibrido(query, top_n=10, alpha=0.5, ciudad=None, df_base=None):
    """
    Búsqueda híbrida TF-IDF + embeddings semánticos (E5 o SBERT).
    Si se pasa df_base, solo busca dentro de ese subconjunto filtrado.
    """

    # --- Seleccionar base de datos ---
    if df_base is not None:
        # Mantener los índices del df global para alinear los scores
    ...
    # --- Filtrar solo los índices locales ---
...
# --- Ejemplo de uso ---
print("🔎 Buscando 'cachopo y sidra' en todo el dataset con modelo MiniLM:")
...
# --- Cargar modelo multilingüe avanzado ---
model = SentenceTransformer("BAAI/bge-m3")

# --- Normalización opcional (recomendado para bge-m3) ---
...
print("⚙️ Calculando embeddings con BAAI/bge-m3 (puede tardar unos minutos la primera vez)...")
embeddings = model.encode(
    df["texto_combinado"].tolist(),
    normalize_embeddings=True,  # bge-m3 soporta normalización interna
    show_progress_bar=True
)
embeddings = np.array(embeddings)

# --- Nueva función híbrida adaptada ---
def buscar_hibrido_avanzado(query, top_n=10, alpha=0.5, ciudad=None):
    ...
    # Combinar scores (ajustando peso: bge-m3 suele ser más fuerte)
...
# --- Ejemplo de uso ---
print("🔎 Buscando 'buenas vistas' en todo el dataset con modelo BGE-M3:")
resultados = buscar_hibrido_avanzado("buenas vistas", top_n=10, alpha=0)
display(resultados)

# --- BLOQUE: Búsqueda híbrida con modelo multilingual-e5-base ---
...
# --- Cargar el modelo E5 ---
# Es multilingüe y entiende frases completas tipo "comida gallega con sidra"
print("⚙️ Cargando modelo multilingual-e5-base (puede tardar unos segundos)...")
model = SentenceTransformer("intfloat/multilingual-e5-base")

# --- Preparar embeddings del corpus (solo se hace una vez) ---
# Añadimos el prefijo "passage: " como recomienda el modelo
...
def buscar_hibrido_e5(query, top_n=10, alpha=0.65, ciudad=None, df_base=None):
    """
    Búsqueda híbrida TF-IDF + multilingual-E5-base.
    Combina coincidencia literal (TF-IDF) y semántica (E5),
    y permite limitar la búsqueda a un subconjunto filtrado (df_base).
    """
    ...
    # --- Filtrar los índices locales (si df_base está definido) ---
    sim_tfidf = sim_tfidf_global[idx_local]
...
# --- Ejemplo de uso ---
print("🔎 Buscando 'buenas vistas' en todo el dataset con multilingual-e5-base:")
...
def limpiar_duplicados(df_in):
    """Elimina duplicados según 'id' o (nombre_final, ciudad)."""
...
PRIORIDAD_MAP = {
    "📌 Coincidencia exacta en texto": 1,     # texto + flags/subcats
    "📌 Coincide con": 2,                     # solo flags/subcats
    "🔎 Coincidencia exacta en texto": 3,     # texto solo
    "✨ Búsqueda semántica": 4,               # búsqueda semántica híbrida
}

def asignar_prioridad(criterio):
    """Asigna prioridad según el tipo de coincidencia textual (parcial)."""
    for clave, valor in PRIORIDAD_MAP.items():
        if clave in str(criterio):  # <-- coincide aunque haya flag_pizza, subcat_asiatica_ramen, etc.
            return valor
    return 99

"""## Búsqueda híbrida + flags/subcategorías

Hasta ahora el sistema combinaba directamente TF-IDF y embeddings semánticos.  
Sin embargo, en muchos casos el usuario espera que las coincidencias **más literales** y con **categorías activas** aparezcan primero.

Por ello se implementa una búsqueda jerárquica con niveles de prioridad:

1. **Coincidencia exacta + flags/subcategorías**  
...
2. **Solo flags/subcategorías**  
   - Si no se llena el `top_n`, se añaden restaurantes que cumplen los filtros activos aunque no mencionen la palabra exacta.  

3. **Coincidencia exacta en texto (sin filtros)**  
   - Si aún faltan resultados, se incluyen los que contienen la palabra en `texto_combinado` aunque no tengan flag/subcategoría activo.  

4. **Búsqueda semántica híbrida**  
   - Como último recurso se completa la lista con la búsqueda híbrida TF-IDF + SBERT, que captura sinónimos y expresiones similares.  

De esta manera se garantiza que los resultados más relevantes para el usuario aparecen al principio, y solo si no hay suficientes se amplía el abanico hacia coincidencias más difusas.
...
def buscar_prioritario(query, flags=None, top_n=50, df_base=None):
    """
    Búsqueda jerárquica prioritaria dentro del DataFrame indicado (df_base).
    ...
    if query.strip() and (main_flags or subcats):
        ...
        if not subset.empty:
            subset["criterio"] = f"📌 Coincidencia exacta en texto ('{query}') + {' + '.join(main_flags + subcats)}"
    ...
    if faltan > 0 and (main_flags or subcats):
        ...
        for f in main_flags + subcats:
            if f in subset.columns:
                subset = subset[subset[f] == 1]
        if not subset.empty:
            subset["criterio"] = f"📌 Coincide con {' + '.join(main_flags + subcats)}"
            resultados = pd.concat([resultados, subset])
        resultados = limpiar_duplicados(resultados)
        faltan = top_n - len(resultados)
    ...
    if faltan > 0 and query.strip():
        ...
        if not subset.empty:
            subset["criterio"] = f"🔎 Coincidencia exacta en texto ('{query}')"
            resultados = pd.concat([resultados, subset])
        resultados = limpiar_duplicados(resultados)
        faltan = top_n - len(resultados)

    # --- Nivel 4: Búsqueda semántica híbrida ---
    if faltan > 0 and query.strip():
        semanticos = buscar_hibrido(query, top_n=faltan, df_base=df_base, ciudad=None).copy()
        semanticos["criterio"] = f"✨ Búsqueda semántica híbrida ('{query}')"
        resultados = pd.concat([resultados, semanticos])
        resultados = limpiar_duplicados(resultados)

    # --- Asignar prioridad única ---
...
"""## Búsqueda única por flags/subcategorías

Además de la búsqueda por texto o proximidad, el sistema permite filtrar restaurantes por categorías:

1. **Flags (`flag_*`)**  
   Variables binarias que representan las categorías principales:  
   - `flag_pizza`  
   - `flag_hamburguesas`  
   - `flag_veggie`  
   - `flag_mexicana`, etc.  

2. **Subcategorías (`subcat_*`)**  
   Algunas categorías maestras tienen subdivisiones más específicas, por ejemplo:  
   - `subcat_española_tapas`  
   - `subcat_española_pulpo`  
   - `subcat_latina_peruana`  
   - `subcat_veggie_sin_gluten`  

### Lógica de filtrado
- Los filtros se aplican en **modo AND**: si se seleccionan varias categorías/subcategorías, el restaurante debe cumplirlas todas.    
- Si no se selecciona ningún flag ni subcategoría, esta capa no elimina resultados.  
...
def filtrar_flags(df, flags_activos=None, top_n=50):
    """Filtra el DataFrame por flags y subcategorías (modo AND)."""
    df_filtrado = df.copy()
    resultados = pd.DataFrame()

    # --- Separar flags principales y subcategorías ---
    ...
    if main_flags or subcats:
        ...
        if not df_filtrado.empty:
            df_filtrado["criterio"] = "📌 Coincide con " + " + ".join(main_flags + subcats)
            resultados = pd.concat([resultados, df_filtrado])

    # --- Si no hay coincidencias ---
    if resultados.empty:
        resultados["criterio"] = "❌ Sin resultados"
...
"""## Función unificada de búsqueda + filtrado

Inicialmente se usaban dos funciones distintas:

- **`buscar_prioritario`** → cuando el usuario introducía una *query* de texto libre.  
- **`filtrar_flags`** → cuando el usuario solo seleccionaba *flags* o *subcategorías*.  

Para simplificar el flujo se crea una función única **`buscar_y_filtrar`** que unifica ambas:

1. Si el usuario introduce una **query** → se ejecuta la lógica de `buscar_prioritario` (con un pre-top de 500 resultados).  
2. Si no hay query → se ejecuta la lógica de `filtrar_flags`.  
3. En ambos casos se aplican filtros adicionales de **ciudad**, **precio máximo** (`price_num`) y **rating mínimo** (`rating_final`).  
4. Finalmente se devuelven los resultados ordenados y limitados al `top_n` solicitado.  

De esta forma, el asistente solo necesita invocar **una función central** para obtener resultados consistentes, independientemente de si la búsqueda se hace por texto libre o solo por categorías.

"""

def buscar_y_filtrar(query="", flags=None, ciudad=None, precio_max=None, rating_min=None, top_n=40):
    """
    Unifica búsqueda textual y filtrado por flags/subcategorías,
    aplicando primero los filtros globales (ciudad, precio, rating)
    y luego la búsqueda jerárquica dentro del subconjunto filtrado.
    """
    ...
    # --- Búsqueda prioritaria dentro del subconjunto filtrado ---
    if query.strip():
...
Además de los filtros por texto y categorías, el sistema permite restringir los resultados a restaurantes **cercanos a la ubicación del usuario**.

En lugar de calcular solo la distancia en línea recta (*Haversine*), utilizamos la API de **OpenRouteService (ORS)**, que devuelve la **distancia real** y el **tiempo estimado de trayecto** en distintos modos de transporte (*coche, andando, bici, etc.*).

### Ventajas
- Distancia real según la red de carreteras o caminos.  
- Tiempo estimado de trayecto en minutos.  
- Geocodificación de direcciones → conversión a coordenadas (lat, lon).  

### Flujo de cálculo
1. Si el usuario proporciona una dirección → se convierte en coordenadas con ORS.  
2. Se hace un **prefiltrado rápido** con Haversine para descartar restaurantes demasiado lejanos.  
3. Para los candidatos cercanos se consulta ORS → obteniendo distancia real y tiempo de trayecto.  
4. Se ordenan los resultados por proximidad o se filtran por radio/tiempo máximo.
...
# --- Paso 3: Proximidad geográfica con OpenRouteService ---

import openrouteservice
from openrouteservice import convert

# ⚠️ Necesitas tu propia API KEY de ORS
...
# --- Función: geocodificar dirección a coordenadas ---
def direccion_a_coordenadas(direccion, solo_espana=True):
    """
    Devuelve las coordenadas (lat, lon) de una dirección mediante ORS Pelias.
    Si solo_espana=True, limita la búsqueda al territorio español
    (previene falsos positivos en direcciones homónimas).
    """
    try:
        extra = {}
        if solo_espana:
            extra = {
                "boundary.country": "ES",
                "boundary.rect.min_lon": -9.30,  # límite oeste
                "boundary.rect.min_lat": 41.80,  # límite sur
                "boundary.rect.max_lon": -6.50,  # límite este
                "boundary.rect.max_lat": 43.80   # límite norte
            }
...
# --- Función: calcular distancia real entre 2 puntos ---
def distancia_real(lat1, lon1, lat2, lon2, modo="driving-car"):
    try:
        ...
        print(f"⚠️ Error en distancia_real con modo {modo}: {e}")
        return None, None
...
# --- Función Haversine (distancia en línea recta en km) ---
def haversine(lat1, lon1, lat2, lon2):
...
def filtrar_proximidad_hibrido(lat, lon, radio_km=5, top_n=5, modo="driving-car"):
    # 1. Preselección con Haversine (rápido, sin API)
    distancias = df.apply(
        ...
        if pd.isna(row["latitude"]) or pd.isna(row["longitude"]):
            if "address_norm" in row and pd.notna(row["address_norm"]):
                ...
                if lat and lon:
                    ...
                    print(f"✅ Coordenadas obtenidas para {row['nombre_final']}: {lat:.5f}, {lon:.5f}")
                else:
                    print(f"⚠️ No se pudo geocodificar la dirección de {row['nombre_final']}")
    return df

# --- Ejemplo de uso ---
# Dirección inicial → centro de Pontevedra
lat_user, lon_user = direccion_a_coordenadas("Praza da Ferrería, Pontevedra, España")
print("📍 Coordenadas usuario:", lat_user, lon_user)
...
Además de los filtros por categorías o proximidad, el sistema permite comprobar si un restaurante está **abierto** en un momento concreto.  

El dataset incluye un campo `horarios_norm` con los horarios en formato string (a veces con guiones largos `–` o cierres en `24:00`).  
Para poder filtrarlos correctamente se siguen estos pasos:

1. **Conversión a lista**  
   - El string se convierte a una lista real mediante `ast.literal_eval`.  

2. **Normalización**  
   - Se sustituyen guiones largos `–` por guiones normales `-`.  
   - Se cambia `24:00` por `23:59` (para que Python lo acepte como hora válida).  

3. **Parseo y comparación**  
   ...
   - Se contemplan casos especiales:
     - *“cerrado”* → se devuelve como cerrado.  
     - *“abierto 24 horas”* → se devuelve como abierto.  
     - *“horario no disponible”* → se devuelve como abierto.  
     - Rango que cruza medianoche (ej: 20:00–02:00) → se considera abierto si la hora cae antes del cierre o después de la apertura.

De esta manera es posible filtrar resultados y mostrar únicamente restaurantes que estén abiertos en el día y hora solicitados.
...
def esta_abierto(horarios, fecha_hora=None):
    ...
    # Si todos son "horario no disponible", también asumimos abierto
    ...
    for h in horarios:
        ...
        if h_norm.startswith(dia_actual):
            ...
            # Normalizar
            horario_str = horario_str.replace("–", "-").replace(" ", "")
            ...
            for r in rangos:
                ...
                try:
                    ...
                    print("⚠️ Error parseando:", r, "->", e)
    return False



# --- Paso 3: función de filtrado ---
...
rest = df.iloc[0]
print("➡️ Restaurante:", rest["nombre_final"])
print("➡️ Horarios:", rest["horarios_lista"])

# probar si está abierto un viernes a las 22:00
fecha_test = datetime.strptime("2025-09-10 00:00", "%Y-%m-%d %H:%M")
print("➡️ ¿Está abierto miércoles a las 00:00?:", esta_abierto(rest["horarios_lista"], fecha_hora=fecha_test))

"""## Función integrada de recomendación + mapa

La lógica completa del recomendador se integra en una única función interactiva **`asistente_recomendador(...)`**, que combina:

1. **Búsqueda semántica híbrida**  
   - Se apoya en TF-IDF + SBERT, con la lógica de prioridad (exactitud → flags/subcategorías → semántica).  

2. **Filtros**  
   - Ciudad, flags y subcategorías, precio máximo (`price_num`) y rating mínimo (`rating_final`).  

3. **Disponibilidad horaria**  
   - Se pueden introducir día y hora concretos, y solo se muestran restaurantes abiertos en ese momento (considerando también casos de *“horario no disponible”* como abiertos).  

4. **Proximidad geográfica**  
   - Si el usuario introduce una dirección, se geocodifica con ORS.  
   - Se hace un prefiltrado rápido con Haversine.  
...
5. **Visualización en mapa**  
   - Los resultados se muestran en un **mapa Folium interactivo** con popups informativos:  
     - Nombre, ciudad, rating, nº de reseñas y precio.  
     - Teléfono, web, menú, reserva online.  
...
De esta forma, el asistente guía al usuario en todo el proceso y devuelve resultados filtrados, ordenados y representados en un mapa interactivo.
...
def mostrar_mapa(resultados, lat_centro=None, lon_centro=None, lat_user=None, lon_user=None):
    if resultados.empty:
        print("❌ No hay resultados para mostrar en el mapa.")
        return None
    ...
    for _, row in resultados.iterrows():
        if "latitude" in row and "longitude" in row and pd.notna(row["latitude"]) and pd.notna(row["longitude"]):
            ...
            <div style="font-size:14px;">
                ...
                📍 {row['ciudad']}<br>
                ⭐ {row['rating_final']} ({row['reviews_final']} reseñas) | 💶 {precio_txt}<br>
                <a href="{row['maps_url_final']}" target="_blank">🌐 Abrir en Google Maps</a><br>
            """

            # --- Teléfono: preferir international_phone ---
            if "international_phone" in row and pd.notna(row["international_phone"]):
                popup_html += f"📞 {row['international_phone']}<br>"
            elif "phone" in row and pd.notna(row["phone"]):
                popup_html += f"📞 {row['phone']}<br>"

            # --- Info extra ---
            if "website" in row and pd.notna(row["website"]):
                popup_html += f"<a href='{row['website']}' target='_blank'>🔗 Web</a><br>"
            if "email" in row and pd.notna(row["email"]):
                popup_html += f"✉️ {row['email']}<br>"
            if "menu_link" in row and pd.notna(row["menu_link"]):
                popup_html += f"<a href='{row['menu_link']}' target='_blank'>📋 Menú</a><br>"
            if "reservation_link" in row and pd.notna(row["reservation_link"]):
                popup_html += f"<a href='{row['reservation_link']}' target='_blank'>📅 Reservar</a><br>"

            # --- Servicios ---
            if "delivery" in row and row["delivery"] == 1:
                popup_html += "🚚 Delivery<br>"
            if "reservable" in row and row["reservable"] == 1:
                popup_html += "📅 Reservable<br>"
    ...
    # 🔎 Leyenda con los mismos iconos que el mapa
    legend_html = """
    <div style="
        position: fixed;
    ...
    <b>🔎 Leyenda</b><br>
    <i class="fa fa-user fa-lg" style="color:red"></i> Tu ubicación<br>
...
def asistente_recomendador():
    print("👋 Bienvenido al recomendador gastronómico")

    # --- Preguntar ciudad ---
    ciudades_disponibles = sorted(df["ciudad"].dropna().str.lower().unique())
    while True:
        ciudad = input(f"\n📍 Ciudades disponibles: {', '.join(ciudades_disponibles)}\n📍 ¿En qué ciudad quieres comer? ").strip().lower()
        if ciudad in ciudades_disponibles:
            break
        print(f"⚠️ Ciudad no disponible: {ciudad}. Opciones válidas: {', '.join(ciudades_disponibles)}")

    # --- Precio máximo ---
    while True:
        precio_txt = input("\n💰 Precio máximo (1=barato ... 4=caro, ENTER = sin Precio máximo): ").strip()
        if not precio_txt:
            precio_max = None
            break
        if precio_txt.isdigit() and 1 <= int(precio_txt) <= 4:
            ...
            break
        print("⚠️ Precio máximo inválido. Debe estar entre 1 y 4.")

    # --- Rating mínimo ---
    while True:
        rating_txt = input("\n⭐ Rating mínimo (ej: 4.0, ENTER = sin Rating mínimo ): ").strip().replace(",", ".")
        if not rating_txt:
            rating_min = None
            break
        try:
            ...
            if 0 <= rating_val <= 5:
                ...
                print("⚠️ Rating inválido. Debe estar entre 0.0 y 5.0")
        except:
            print("⚠️ Introduce un número válido (ej: 4.2).")

    # --- Flags (filtro principal) ---
    flags_disponibles = [c.replace("flag_", "") for c in df.columns if c.startswith("flag_")]

    print("\n📋 Categorías gastronómicas disponibles y sus subcategorías:")
    for f in sorted(flags_disponibles):
        ...
        if posibles_subcats:
            ...
            print(f"   - {maestro} → [{subcats_txt}]")
        else:
            print(f"   - {maestro}")

    while True:
        flags_txt = input(
            "🍔 Indica una o varias categorías separadas por coma (ej: pizza, veggie, italiana). ENTER = ninguna: "
        ).strip()
        ...
        if flags_invalidas:
            print(f"⚠️ Las siguientes categorías no son válidas: {', '.join(flags_invalidas)}")
            print("💡 Revisa la lista anterior e intenta de nuevo.\n")
            continue  # vuelve a pedir toda la lista

        if not flags_input:
            print("⚠️ No se reconoció ningún filtro válido. Inténtalo de nuevo.\n")
            continue

        # Si todo es válido, convertir a formato flag_ y respetar el orden
        flags = ["flag_" + f for f in flags_input]
        print(f"✅ Categorías seleccionadas: {', '.join(flags_input)}")
        break
    ...
    if flags:
        for f in flags.copy():
            ...
            if posibles_subcats:
                ...
                print(f"\n📋 Subcategorías disponibles para {maestro}:")
                for sc in subcats_limpias:
                    print(f"   - {sc}")
                while True:
                    subcat = input(f"👉 ¿Quieres alguna subcategoría de {maestro}? (ENTER = sin subcategoría): ").strip().lower().replace(" ", "_")
                    if not subcat:
                        break
                    elif f"subcat_{maestro}_{subcat}" in df.columns:
                        flags.append(f"subcat_{maestro}_{subcat}")
                        print(f"✅ Subcategoría añadida: {subcat}")
                        break
                    else:
                        print(f"⚠️ Subcategoría no válida: {subcat}.")
                        print("💡 Revisa la lista anterior e intenta de nuevo.\n")


    # --- Query (texto libre) ---
    query = input("\n🔎 ¿Quieres afinar con un término libre (ej: terraza, barato, menú degustación...)? ENTER = ninguno: ").strip()

    # --- Preguntar si quiere filtrar por día y horario ---
    usar_horarios = None
    while usar_horarios not in ["s", "n"]:
        usar_horarios = input("\n📅 ¿Quieres filtrar por día y horario de apertura? (s/n): ").strip().lower()
        if usar_horarios not in ["s", "n"]:
            print("⚠️ Respuesta no válida. Escribe 's' o 'n'.")
    ...
    if usar_horarios:
        ...
        # --- Pedir día ---
        while True:
            dia_txt = input("\n👉 Día de la semana (ej: viernes, ENTER = hoy): ").strip().lower()
            if not dia_txt:
                # si el usuario da ENTER, se usa el día actual
                ...
                print("⚠️ Día no válido. Debe ser uno de:", ", ".join(dias))

        # --- Pedir hora ---
        while True:
            hora_txt = input("\n👉 Hora en formato 24h (ej: 22:00, ENTER = ahora): ").strip().replace(".", ":")
            if not hora_txt:
            ...
            try:
                ...
                print("⚠️ Hora no válida. Usa el formato HH:MM (por ejemplo, 21:30).")
    ...
    while True:
        direccion_usuario = input(
            "\n🚗 Introduce tu dirección (ej: 'Rúa do Franco 10, Santiago de Compostela, A Coruña') "
            "ENTER = sin dirección: "
        ).strip()

        if not direccion_usuario:
            print("ℹ️ No se indicó dirección. Se continuará sin ubicación.")
            break

        lat_user, lon_user = direccion_a_coordenadas(direccion_usuario)
        if lat_user and lon_user:
            print(f"✅ Dirección reconocida: {direccion_usuario} → ({lat_user:.5f}, {lon_user:.5f})")
            break
        else:
            intentos += 1
            print("⚠️ No se pudo reconocer la dirección.")
            if intentos < 3:
                print("💡 Prueba a escribirla con más detalle (ejemplo: 'Calle Real 5, A Coruña, Galicia').\n")
            else:
                print("❌ No se pudo obtener la ubicación tras varios intentos. Se continuará sin ubicación.")
                lat_user, lon_user = None, None
                break

    # --- Ordenar proximidad (solo si hay dirección) ---
    ordenar_por_proximidad = False

    if lat_user and lon_user:
        while True:
            respuesta = input("📍 ¿Quieres ordenar los resultados por cercanía en coche? (s/n): ").strip().lower()

            if respuesta in ["s", "n"]:
                ordenar_por_proximidad = (respuesta == "s")
                break

            print("\n⚠️ Respuesta no válida. Escribe 's' para sí o 'n' para no.")
    ...
    # --- Depuración: verificar prioridades ---
    if "criterio" in resultados.columns:
        print("\n🧩 Depuración de prioridades:")
    ...
    if not resultados.empty and lat_user and lon_user:
        ...
        for _, row in candidatos.iterrows():
            try:
                ...
                time.sleep(1)  # ✅ ahora correcto
            except Exception as e:
                print(f"⚠️ Error calculando distancias para {row['nombre_final']}: {e}")
        ...
        if ordenar_por_proximidad:
            # Insertamos distancia después de la prioridad, antes del rating
    ...
    if resultados.empty:
        ...
        for _, row in resultados.head(15).iterrows():
            car_info = f" 🚗 {row['distancia_car_km']:.1f} km | {row['tiempo_car_min']:.0f} min" if pd.notna(row.get("distancia_car_km")) else ""
            walk_info = f" 🚶 {row['distancia_walk_km']:.1f} km | {row['tiempo_walk_min']:.0f} min" if pd.notna(row.get("distancia_walk_km")) else ""

            print(f"➡️ {row['nombre_final']} ({row['ciudad']}) ⭐ {row['rating_final']} "
                  f"({row['reviews_final']} reseñas) 💰 {row['price_num']}{car_info}{walk_info}\n"
                  f"   {row['maps_url_final']}\n"
                  f"   📝 Este restaurante se ha elegido porque cumple: {row['criterio']}\n")
...
print("✅ Dataset enriquecido:", df_enriquecido.shape)
